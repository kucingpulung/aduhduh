...
@Client.on_message(filters.command("setcaption") & filters.user(authorized_users_only))
async def set_caption_handler(client: Client, message: Message):
    if not message.reply_to_message or not message.reply_to_message.text:
        return await message.reply("Balas pesan ini dengan template caption.\nGunakan `{original_caption}` sebagai placeholder.")
...
@Client.on_message(filters.command("getcaption") & filters.user(authorized_users_only))
...
@Client.on_message(filters.command("resetcaption") & filters.user(authorized_users_only))
